...
function doctorschedule() {
  ...
  return (
    <div>
      ...
      <div className='grid grid-flow-col auto-cols-6 gap-8 items-center   bg-gray-200 rounded-md px-4 py-10'>
        <form>
          <label className='text-xl'>Doctor : </label><br></br>
        <select onChange={(e)=>{setDocname(e.target.value)}} className='w-[200px] h-[30px] border-2 rounded-lg text-xl' >
                     
            {alldoctor.map((d,index)=>(<option key={index} value={d.name}>{d.name}-{d.id}</option>))}
            </select>
        </form>     
        <form>
          <label className='text-xl'>Week : </label><br></br>
          <select  onChange={(e)=>setWeek(e.target.value)} className='text-xl border-2 w-[70px] border-gray-600 rounded-md'>
        ...
        <form>
          <label className='text-xl'>Duty : </label><br></br>
          <select  onChange={(e)=>setDuty(e.target.value)} className='text-xl border-2 w-[135px] border-gray-600 rounded-md'>
        ...
        <form>
          <label className='text-xl'>Date :</label><br></br>
          <input onChange={(e)=>setDate(e.target.value)} className='w-[140px] text-lg shadow h-[30px] border-2 border-black' type="date"></input>
        </form>
        <form>
          <label className='text-xl'>Time Slot : </label><br></br>
          <select  onChange={(e)=>setTimeslot(e.target.value)} className='text-xl border-2 w-[220px]  border-gray-600 rounded-md'>
      ...
      <button  onClick={Fetch} className='mt-4 ml-[181px] mb-4 w-[350px]  rounded-lg h-[40px]   bg-green-600 font-bold' type="submit">Submit</button> 
      <button  className='mt-4 ml-10 mb-4 w-[350px]  rounded-lg h-[40px]   bg-red-600 font-bold' type="submit">Cancel</button> <br></br><br></br>
